...
export function txSlice(
  ...
  switch (timeframe) {
    case "1M":
      ...
      break;
    case "3M":
      startStr = now.addMonths(-2).month();
      break;
    case "6M":
      startStr = now.addMonths(-5).month();
      break;
    case "1Y":
      startStr = now.addMonths(-11).month();
      break;
    case "YTD":
      startStr = `${curYear}-01`;
      break;
...
export function bestSaving(
  ...
  const months = chartMonths.map(({ year, month }) => {
    ...
    return {
      label: `${d.short()} ${year !== curYear ? year : ""}`.trim(),
      net,
    };
  });
  return months.reduce((best, cur) => (cur.net > best.net ? cur : best), {
    label: "—",
    net: 0,
  });
}
