...
import { txSlice, topBurn, bestSaving } from "./metric-fns";
...
export function worthSparkline({ transactions }: MetricData): Array<{ month: string; value: number }> {
  const now = Dater.now();
...
export function worthChange({ transactions }: MetricData): number {
  ...
  const thisNet = transactions.filter((t) => t.date.startsWith(thisMs)).reduce((s, t) => s + t.amount, 0);
  const lastNet = transactions.filter((t) => t.date.startsWith(lastMs)).reduce((s, t) => s + t.amount, 0);
...
export function burnSparkline({ transactions }: MetricData): Array<{ day: string; value: number }> {
  const ago = Dater.now().addDays(-30).iso();
...
export function catBreakdown({ transactions }: MetricData): Array<{ label: string; amount: number; percent: number }> {
  const ago = Dater.now().addDays(-30).iso();
  const recent = transactions.filter((t) => t.date >= ago && t.amount < 0);
  const map = new Map<string, number>();
  recent.forEach((t) => {
...
export function catSpend({ transactions }: MetricData): number {
  const ago = Dater.now().addDays(-30).iso();
  return transactions
    .filter((t) => t.date >= ago && t.amount < 0 && t.categoryId && t.categoryId !== "transfer" && t.categoryId !== "income")
    .reduce((s, t) => s + Math.abs(t.amount), 0);
}

// ─── Reports ───────────────────────────────────────────────────────────────────

function chartMonths(timeframe: Timeframe): Array<{ year: number; month: number }> {
  const now = Dater.now();
  ...
  switch (timeframe) {
    ...
    case "3M": { const d = now.addMonths(-2); startYear = d.year(); startMonth = d.monthIndex(); break; }
    case "6M": { const d = now.addMonths(-5); startYear = d.year(); startMonth = d.monthIndex(); break; }
    case "1Y": { const d = now.addMonths(-11); startYear = d.year(); startMonth = d.monthIndex(); break; }
...
export function chartData({ accounts, transactions }: MetricData, { timeframe = "YTD" }: MetricOptions): MonthlyNetWorth[] {
  ...
  return [
    ...months
      .filter(({ year, month }) => tx.some((t) => t.date.startsWith(Dater.ofYM(year, month).month())))
      .map(({ year, month }) => {
        ...
        return {
          ...
          ...(year === curYear && month === curMonth ? { isCurrent: true as const } : {}),
        };
      }),
    projected,
  ];
...
export function ytdChange({ accounts, transactions }: MetricData): number {
  ...
  const now = Dater.now();
  const currentNW = accounts.reduce((s, a) => s + a.balance, 0);
  const ytdNet = tx.filter((t) => t.date.slice(0, 7) >= `${now.year()}-01`).reduce((s, t) => s + t.amount, 0);
  const startNW = currentNW - ytdNet;
  return startNW > 0 ? Math.round(((currentNW - startNW) / Math.abs(startNW)) * 1000) / 10 : 0;
}

export function income({ transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
...
export function expenses({ transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  return txSlice(tx, opts.timeframe ?? "YTD")
    .filter((t) => t.amount < 0)
    .reduce((s, t) => s + Math.abs(t.amount), 0);
...
export function savingsRate({ transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  const window = txSlice(tx, opts.timeframe ?? "YTD");
  const inc = window.filter((t) => t.amount > 0).reduce((s, t) => s + t.amount, 0);
  const exp = window.filter((t) => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0);
  return inc > 0 ? Math.round(((inc - exp) / inc) * 1000) / 10 : 0;
}

export function savingsChange({ transactions }: MetricData, opts: MetricOptions): number {
  ...
  const { year: sy, month: sm } = months[0] ?? { year: now.year(), month: now.monthIndex() };
  ...
  const pInc = prevWindow.filter((t) => t.amount > 0).reduce((s, t) => s + t.amount, 0);
  const pExp = prevWindow.filter((t) => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0);
  const prev = pInc > 0 ? Math.round(((pInc - pExp) / pInc) * 1000) / 10 : 0;

  return Math.round((current - prev) * 10) / 10;
}

export function topCategory({ transactions }: MetricData, opts: MetricOptions): string {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  return topBurn(txSlice(tx, opts.timeframe ?? "YTD"))?.name ?? "—";
}

export function topAmount({ transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  return topBurn(txSlice(tx, opts.timeframe ?? "YTD"))?.amount ?? 0;
}

export function bestMonth({ transactions }: MetricData, opts: MetricOptions): string {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  const now = Dater.now();
...
export function bestSaved({ transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  const now = Dater.now();
  const months = chartMonths(opts.timeframe ?? "YTD");
...
export function runway({ accounts, transactions }: MetricData, opts: MetricOptions): number {
  const tx = transactions.filter((t) => t.categoryId !== "transfer");
  const currentNW = accounts.reduce((s, a) => s + a.balance, 0);
  const months = chartMonths(opts.timeframe ?? "YTD");
  const window = txSlice(tx, opts.timeframe ?? "YTD");
