...
const config: Record<AssetType, { label: string; bar: string; dot: string }> = {
  ...
  "real-estate": { label: "Real Estate", bar: "bg-orange-400", dot: "bg-orange-400" },
...
export function AllocationBar({ byType, currency }: AllocationBarProps) {
  const slices = (Object.entries(byType) as [AssetType, { value: number; pct: number }][])
    ...
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-3">
      ...
      {slices.map(([type, { value, pct }]) => (
        <span key={type} className="inline-flex items-center gap-1.5">
          <span className={`size-1.5 rounded-full ${config[type].dot}`} aria-hidden="true" />
